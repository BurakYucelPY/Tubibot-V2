"""
TÜBİTAK Haber Scraper
https://tubitak.gov.tr/tr/haberler adresinden son 5 sayfayı çeker.
Sonuçları backend/haberler.json dosyasına yazar.
"""
...
PAGES = 5
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "haberler.json")
...
def run_scraper() -> list[dict]:
    ...
    for page_num in range(PAGES):
        ...
        print(f"[HABER] Sayfa çekiliyor: {url}")
        soup = fetch_page(url)
        if not soup:
            continue
    ...
    print(f"\n[HABER] {len(all_items)} haberin detay sayfası çekiliyor...")
    ...
    print(f"\n[OK] {len(all_items)} haber -> {OUTPUT_PATH}")
    return all_items
