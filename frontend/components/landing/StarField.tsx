...
const constellations: Array<Array<StarPoint>> = [
  [
    [247, 103],
    [261, 86],
    [307, 104],
...
function Constellation({
  ...
  useEffect(() => {
    ...
    const strokeDelay = Math.random() * 3 + 2;
    const strokeDuration = 5;

    const strokeAnimation = animate(
      node,
      { strokeDashoffset: 0, visibility: "visible" },
      { duration: strokeDuration, delay: strokeDelay },
    );

    let fillAnimation: ReturnType<typeof animate> | undefined;
    if (isFilled) {
      fillAnimation = animate(
        node,
        { fill: "rgb(255 255 255 / 0.02)" },
        { duration: 1, delay: strokeDelay + strokeDuration },
      );
    }
